...
export default function App() {
  // 📝 [參考綁定] 用來綁定 DOM 元素與儲存跨渲染週期的實例
  const pixiContainer = useRef(null); // PIXI 畫布的掛載點
  const videoRef = useRef(null);      // 隱藏的 HTML5 Video 元素 (供 AI 辨識用)
  const engineRef = useRef(null);     // 儲存 PIXI 引擎實例 (SakuraInkEngine)
  const eyeCoordsRef = useRef(null);  // 儲存最新的眼眶座標，供 PIXI 引擎隨時取用
  
  // 📝 [狀態管理] 控制整個互動體驗的生命週期
  // 狀態包含：'init'(初始) -> 'loading'(載入AI) -> 'ready'(選單) -> 'playing'(播放中) -> 'finished'(結算)
  const [interactionState, setInteractionState] = useState('init');
  const [selectedWords, setSelectedWords] = useState([]); // 使用者選擇的情緒詞彙陣列

  // 📝 [生命週期] 元件掛載時，動態載入 PIXI.js 核心函式庫
  useEffect(() => {
  ...
  const initCameraAndAI = async () => {
    ...
    try {
      // 📝 載入 Google MediaPipe Face Landmarker 模型
      ...
      // 📝 啟動 PIXI 渲染引擎，並傳入抓取眼球座標的 function 與結束時的 callback
      ...
      setInteractionState('ready'); // 進入等待使用者操作狀態
    } catch (err) { console.error(err); setInteractionState('init'); }
  };

  // 📝 [AI 追蹤迴圈] 每一幀分析影像，並更新眼眶座標供 PIXI 使用
    ...
    const loop = () => {
      if (videoRef.current && videoRef.current.currentTime !== lastVideoTime) {
        ...
        if (results.faceLandmarks && results.faceLandmarks.length > 0) {
          ...
          // 計算 AI 座標映射到 PIXI 螢幕尺寸的縮放比例
          const scale = Math.min(400 / vw, MONITOR_H / vh);
          
          // 📝 座標映射轉換：將 AI 的 0~1 比例轉換為螢幕實際像素位置 (含鏡像反轉)
          const mapPoint = (mark) => {
