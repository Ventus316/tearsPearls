...
export function createInkEngine(containerElement, getEyeData, videoElement, onComplete) {
  ...
  gemGlow.beginFill(0xF59E0B, 0.4); gemGlow.drawCircle(0, 0, 55); gemGlow.endFill();
  ...
  gemCore.lineStyle(2, 0xFDE68A, 0.9); gemCore.beginFill(0xD97706, 1.0);
  ...
  let wasActive = false; 
  let currentWordPool = WORDS; // 預設使用全部

  // 新增參數：userWords
  const spawnWordFlow = (userWords, isInner = Math.random() > 0.5, sizeScale = 1.0) => {
    showGem = false; hasTriggeredComplete = false; gemReadyTimer = 0;
    ...
    chars.forEach((char, index) => {
      dropQueue.push({ char, x: eyeX, y: eyeY, triggerFrame: frameCounter + (index * WORD_SPAWN_INTERVAL), scale: sizeScale });
    });
  };
  ...
  app.ticker.add((delta) => {
    ...
    for (let i = drops.length - 1; i >= 0; i--) {
      ...
      if (depthRatio > FADE_START_RATIO) { drop.sprite.alpha += ((1 - ((1 - MIN_ALPHA) * fadeProgress)) - drop.sprite.alpha) * ALPHA_EASE; }
      if ((drop.sprite.y - drop.lastTrailY) >= Math.max(3, TRAIL_SPAWN_DENSITY * drop.baseScale) && depthRatio > TRAIL_START_DEPTH) {
        ...
        trail.anchor.set(0.5); trail.position.set(drop.sprite.x, drop.sprite.y); trail.rotation = Math.random() * 0.2 - 0.1; trail.scale.set((TRAIL_SCALE_X_BASE + (depthRatio * TRAIL_SCALE_X_DEPTH_MULTIPLIER)) * drop.baseScale, TRAIL_SCALE_Y * drop.baseScale); 
        const trailBlur = new window.PIXI.BlurFilter(); trailBlur.blur = depthRatio * TRAIL_INITIAL_BLUR_MULTIPLIER; trail.filters = [trailBlur]; trail.alpha = TRAIL_BASE_ALPHA + (depthRatio * TRAIL_DEPTH_ALPHA_MULTIPLIER); trailContainer.addChildAt(trail, 0);
      ...
      if (drop.sprite.y > screenBottom) {
          if (drop.screen === 1) { tabletQueue.push({ char: drop.char, x: drop.sprite.x, scale: drop.baseScale, vx: drop.vx, vy: drop.vy, triggerFrame: frameCounter + NETWORK_DELAY_FRAMES }); textContainer.removeChild(drop.sprite); drop.sprite.destroy(); drops.splice(i, 1); }
          else { drop.isConverging = true; }
      }
    }
  ...
  return {
    spawnWord: (userWords) => { currentWordPool = userWords; showGem = false; hasTriggeredComplete = false; gemReadyTimer = 0; targetGemScale = 0; spawnWordFlow(userWords, Math.random() > 0.5, 0.8); },
    triggerCryingSequence: (userWords) => { if(!isCrying) { currentWordPool = userWords; showGem = false; hasTriggeredComplete = false; gemReadyTimer = 0; targetGemScale = 0; isCrying = true; cryingTime = 0; wordSpawnTimer = 0; } },
